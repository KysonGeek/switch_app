...
./build_app.sh            # 编译 + 打包 + 签名，产出「WindowSwitcher.app」
open "WindowSwitcher.app"  # 启动，菜单栏出现 ▱ 图标
```
...
- 临时签名本身**不会过期**，纯本地构建也不会被 Gatekeeper 拦（除非把 `.app` 经网络/AirDrop 传到别处，那时 `xattr -dr com.apple.quarantine "WindowSwitcher.app"` 即可）。
...
# 并在 系统设置 ▸ 辅助功能 里移除「WindowSwitcher」条目
```
...
> CI 上没有本地签名证书，产物用 ad-hoc 签名；下载解压后首次打开会被 Gatekeeper 拦，执行 `xattr -dr com.apple.quarantine "WindowSwitcher.app"` 即可。
