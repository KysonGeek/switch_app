...
- **绑定优先级**：窗口按 中排(home) → 上排 → 下排 → 数字行 的顺序分配到最好按的键
...
./build_app.sh            # 编译 + 打包 + 签名，产出「窗口闪切.app」
open "窗口闪切.app"        # 启动，菜单栏出现 ▱ 图标
```
...
- 临时签名本身**不会过期**，纯本地构建也不会被 Gatekeeper 拦（除非把 `.app` 经网络/AirDrop 传到别处，那时 `xattr -dr com.apple.quarantine "窗口闪切.app"` 即可）。
